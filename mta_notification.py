...
if time_until_train > 10:
	...
	print("The next Manhattan-bound F train from Fort Hamilton Station arrives in")
	print(time_until_train) 
	print("minutes at ")
	print(time.strftime("%I:%M %p", time.localtime(nearest_arrival_time)))
elif time_until_train <= 0:
	print("You *just* missed the train. Ah well, the next train will arrive at") 
	print(time.strftime("%I:%M %p", time.localtime(second_arrival_time)))
else:
	print("HURRY UP YOU HAVE ")
	print(time_until_train) 
	print("MINUTES TO GET TO FORT HAMILTON! THE TRAIN GETS IN AT")
